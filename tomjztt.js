...
async function all() {
    ...
    if(DATA.code == 0){
        ...
        //await ssp()
        await task(`get`, 'https://api.st615.com/v1/user/task?token='+jztoken,headerss)
        data = DATA;
        //console.log(JSON.stringify(data.data));
        if(data.data.daily_task[2].id == 6 && data.data.daily_task[2].is_finish != 1){
            await task(`post`, 'https://api.st615.com/v1/task/clock',headerss,`is_double=0&token=${jztoken}`)
            data = DATA
            console.log(data.data);
            await $.wait(3000)
            await task(`post`, 'https://api.st615.com/v1/task/continue',headerss,`token=${jztoken}`)
            data = DATA
            console.log(data.data);
            await $.wait(3000)
            await task(`post`, 'https://api.st615.com/v1/sign/sign',headerss,`token=${jztoken}`)
            data = DATA
            console.log(data.data);
            await $.wait(3000)
            await task(`post`, 'https://api.st615.com/v1/task/ads',headerss,`id=94&token=${jztoken}`)
            data = DATA
            console.log(data.data);
            await $.wait(3000)
        }
        await task(`get`, `https://api.st615.com/v1/article/list?type=2&cid=0&page=1&terminal=Apple&version=1.2.8&token=${jztoken}`,headerss)
        spdata = DATA
        //console.log(JSON.stringify(data.data));
        await $.wait(3000)
        if(data.code==0){
            for(let i = 0; i < 10; i++){
                spids = spdata.data.list[i].id
                //console.log(spids);
                await task(`post`, `https://api.st615.com/v1/article/finish`,headerss,`id=${spids}&token=${jztoken}`)
                data = DATA
                if(data.code == 0){console.log(JSON.stringify(data));}
