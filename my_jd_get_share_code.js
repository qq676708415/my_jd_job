...
$.JdCFD=[];
$.JdCash=[];
async function joinShareCode() {

    console.log(`\n\n======合并互助码 (共${$.indexMax}个京东帐号)======`)

    await joinShareCodeFix($.JdFruit,`京东农场`,3,5);
    await joinShareCodeFix($.JdPet,`京东萌宠`,5,5);
    await joinShareCodeFix($.JdPlantBean,`种豆得豆`,3,9);
    await joinShareCodeFix($.JdFactory,`东东工厂`,3,5);
    await joinShareCodeFix($.JxFactory,`京喜工厂`,3,10);
    await joinShareCodeFix($.JxNC,`京喜农场`,3,5);
    // await joinShareCodeFix($.JdZZ,`京东赚赚`,3,5);
    // await joinShareCodeFix($.JdJoy,`京东Joy`,3,5);
    await joinShareCodeFix($.JdSgmh,`闪购盲盒`,3,10);
    // await joinShareCodeFix($.JdCFD,`财富岛`,3,5);
    await joinShareCodeFix($.JdCash,`领现金`,3,10);

    console.log(`======合并互助码结束======\n`)
}

async function joinShareCodeFix(shareList,shareName,shareMax,shareNeed) {
    var list=[];
    shareList= shareList||[];
    shareFist = shareList.length>0?shareList[0].ShareCode:"NO";
    for(var i=1;i<=$.indexMax;i++){
        _start = parseInt( i/(shareNeed+1));
        //_shareArr =  shareList.map((item)=>{ return item.index!=i?item.ShareCode:null;}).filter((item)=>{return item!=null}).slice(_start,shareMax);
        _shareArr =  shareList.filter((item)=>{ return item.index!=i}).map((item)=>{return item.ShareCode}).slice(_start,shareMax);
        if(_shareArr.length<=0) _shareArr.push(shareFist);
        list.push(_shareArr.join('@'));
    }
    console.log(`${shareName}互助码：\n${list.join('\n')}`);
}
              ...
              if (data.data.bizCode === 0) {
                ...
                $.taskVos.map((item) => {
                  if (item.taskType === 14) {
                    ...
                    $.JdFactory.push({index:$.index,UserName:$.UserName,ShareCode:item.assistTaskDetailVo.taskToken});
                  }
                });
              }
              else{
                console.log(`${data.data.bizMsg}\n`);
                //{"code":0,"data":{"bizCode":-1001,"bizMsg":"活动太火爆啦","success":false},"msg":"调用成功"}
              }
            }
          }
        } catch (e) {
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                ...
                if (data.factoryList && data.productionList) {
                  ...
                  $.JxFactory.push({index:$.index,UserName:$.UserName,ShareCode:data.user.encryptPin});
                }
              } else {
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                if (data.active) {
                  ...
                  $.JxNC.push({index:$.index,UserName:$.UserName,ShareCode:JSON.stringify(shareCodeJson)});
        ...
        if (err) {
          ...
          if (
            ...
            if ($.petInfo.userStatus === 0) {
              console.log( `【提示】京东账号${$.index}${$.nickName}萌宠活动未开启请手动去京东APP开启活动入口：我的->游戏与互动->查看更多开启`);
            }
            else{
                console.log(`【京东账号${$.index}（${$.UserName}）京东萌宠】${$.petInfo.shareCode}`);
                $.JdPet.push({index:$.index,UserName:$.UserName,ShareCode:$.petInfo.shareCode});
            }
          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if ($.taskList.filter(item => !!item && item['taskId']=== 3) && $.taskList.filter(item => !!item && item['taskId']=== 3).length) {
                ...
                var _shareCode=$.taskList.filter(item => !!item && item['taskId']=== 3)[0]['itemId'];
                $.JdZZ.push({index:$.index,UserName:$.UserName,ShareCode:_shareCode});
              }
              else
              {
                console.log(`【京东账号${$.index}（${$.UserName}）的京东赚赚好友互助码】获取失败（taskId===3）`);
              }
            }
          }
        } catch (e) {
...
async function getPlantBean() {
  ...
  async function jdPlantBean() {
    ...
    if ($.plantBeanIndexResult.code === "0") {
      ...
      $.JdPlantBean.push({index:$.index,UserName:$.UserName,ShareCode:$.myPlantUuid});
    } else {
      console.log(
...
async function getJDFruit() {
  ...
  async function jdFruit() {
    ...
    if ($.farmInfo.farmUserPro) {
      ...
      $.JdFruit.push({index:$.index,UserName:$.UserName,ShareCode:$.farmInfo.farmUserPro.shareCode});
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.success && data.data && data.data.userInviteCode) {
              ...
              $.JdJoy.push({index:$.index,UserName:$.UserName,ShareCode:data.data.userInviteCode});
            }
            else{
                console.log(`【京东账号${$.index}（${$.UserName}）crazyJoy】${data.message}`)
            }
          }
        }
      } catch (e) {
      ...
      $.post(url, async (err, resp, data) => {
        try {
          ...
          if (data.data.bizCode === 0) {
            ...
            {
                _shareCode= invites[0].assistTaskDetailVo.taskToken;
                console.log(`【京东账号${$.index}（${$.UserName}）闪购盲盒】${_shareCode}`);
                $.JdSgmh.push({index:$.index,UserName:$.UserName,ShareCode:_shareCode});
            }
            else{ 
                console.log(`【京东账号${$.index}（${$.UserName}）闪购盲盒】${data.data.bizMsg}`);
            }
          }
        } catch (e) {
          $.logErr(e, resp);
  ...
  return new Promise(async (resolve) => {
    $.get(taskUrl(`user/QueryUserInfo`), (err, resp, data) => {
      try {
        ...
        $.JdCFD.push({index:$.index,UserName:$.UserName,ShareCode:`${strMyShareId}`});

      } catch (e) {
        console.log(`【京东账号${$.index}（${$.UserName}）财富岛】 catch ${e.message}`)
        //$.logErr(e, resp);
      } finally {
        resolve();
      }
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if(data.code===0 && data.data.result){
              ...
              $.JdCash.push({index:$.index,UserName:$.UserName,ShareCode:data.data.result.inviteCode});
